...
from utils.misc import color_filter, erode_to_black, erode_to_black2
...
class cropResponse:
    def __init__(self, key, max_idx, cropped_item, x, y, w, h):
        self.key = key
        self.max_idx = max_idx
        self.cropped_item = cropped_item
        self.x = x
        self.y = y
        self.w = w
        self.h = h
...
class ItemCropper:
    ...
    def clean_img(self, inp_img: np.ndarray, black_thresh: int = 14) -> np.ndarray:
        ...
        img = erode_to_black2(img, black_thresh)
        return img

    def process_crop(self, key, cleaned_img, padding_y ):
        output = []
        ...
        for cntr in contours:
            ...
            if contains_black and mostly_dark and expected_height and expected_width:
                ...
                if key == self._item_colors[max_idx]:
                    output.append(cropResponse(key, max_idx, cropped_item, x, y, w, h))
                    #return cropResponse(key, max_idx, cropped_item, x, y, w, h)
    ...
    def crop(self, inp_img: np.ndarray, padding_y: int = 5) -> list[ItemText]:
        ...
        start = time.time()
        item_clusters = []
        thrd = [0]*len(self._item_colors)
        pool = ThreadPool(processes=10)
        for i, key in enumerate(self._item_colors):
            thrd[i] = pool.apply_async(self.process_crop, (key, cleaned_img, padding_y))


        for i, key in enumerate(self._item_colors):
            if isinstance(thrd[i],int): continue #skip ded threads
            output = thrd[i].get()
            if output is not None:
                for item in output:
                    item_clusters.append(ItemText(
                                color = item.key,
                                roi = [item.x, item.y, item.w, item.h],
                                data = item.cropped_item,
                                clean_img = cleaned_img[item.y:item.y+item.h, item.x:item.x+item.w]
                            ))
    ...
    def crop2(self, inp_img: np.ndarray, padding_y: int = 5) -> list[ItemText]:
        start = time.time()
        cleaned_img = self.clean_img(inp_img)
        debug_str = f" | clean: {time.time() - start}"
