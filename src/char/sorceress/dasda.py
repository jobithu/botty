...
class BlizzSorc(Sorceress):
    ...
    def kill_cs_trash(self) -> bool:
        ...
        self._ice_blast((30, -50), spray=180)
        wait(0.5)
        self._cast_static()
        #move down
    ...
    def kill_vizier(self, nodes1: list[int], nodes2: list[int]) -> bool:
        ...
        self._ice_blast((10, 10), spray=120)
        self._cast_static()
        #move down
        ...
        self.move(pos_m, force_move=True)
        self._blizzard((0, 0), spray=10)
        self._ice_blast((70, -230), spray=20)
        wait(0.5)
        pos_m = self._screen.convert_abs_to_monitor((100, -50))
        self.pre_move()
        self.move(pos_m, force_move=True)
    ...
    def kill_deseis(self, nodes1: list[int], nodes2: list[int], nodes3: list[int]) -> bool:
        #bait teledance
        if seal_layout == "B1-S":
            self._pather.traverse_nodes([632], self, force_tp=True)
            self._cast_static()
            self._pather.traverse_nodes([633], self, force_tp=True)
            pos_m = self._screen.convert_abs_to_monitor((10, -85))
            self.pre_move()
            self.move(pos_m, force_move=True)
            self._blizzard((0, 0), spray=10)
            self._ice_blast((0, 0), spray=360)
            self._blizzard((0, 0), spray=10)
            self._ice_blast((0, 0), spray=360)
            self._cast_static()
            #move down
            ...
            pos_m = self._screen.convert_abs_to_monitor((35, -80))
            self.pre_move()
            self.move(pos_m, force_move=True)
            #bait teledance
            self._cast_static()
            ...
            self.pre_move()
            self.move(pos_m, force_move=True)
            self._blizzard((50, -200), spray=10)
            self._ice_blast((230, 230), spray=20)
            wait(0.5)
            self._cast_static()
            pos_m = self._screen.convert_abs_to_monitor((-15, -100))
            ...
            self._pather.traverse_nodes([641], self, force_tp=True)
            self._cast_static()
            pos_m = self._screen.convert_abs_to_monitor((10, -85))
            self.pre_move()
            self.move(pos_m, force_move=True)
