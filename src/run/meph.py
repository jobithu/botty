    ...
    def battle(self, do_pre_buff: bool) -> Union[bool, tuple[Location, bool]]:
        ...
        if layout == 'WPTHREE':
            ...
            if template_match.valid:
                ...
                self._char.move(pos_m, force_tp=True, force_move=True)
            while not roomfound:
                roomfound = self._template_finder.search_and_wait(["MEPH_LVL2_WP4E_1", "MEPH_LVL2_WP4E_00", "MEPH_LVL2_WP4E_0", "MEPH_LVL2_WP4E_2", "MEPH_LVL2_WP4E_3", "MEPH_LVL2_WP4E_4"], threshold=0.65, time_out=0.1, take_ss=False, use_grayscale=False).valid
                t0 = self._screen.grab()
                pos_m = self._screen.convert_screen_to_monitor(template_match.position)
                self._char.move(pos_m, force_tp=True, force_move=True)
                t1 = self._screen.grab()
