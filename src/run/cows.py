...
class Cows:
    ...
    #from ui_manager
    # def buy_tome(self, curr_loc: Location) -> Union[Location, bool]:
    ...
    def _open_cow_portal(self)-> bool:
        ...
        keyboard.send('ctrl', do_press=False)
        self._ui_manager.close_vendor_screen()
        # curr_loc = TownManager.get_act_from_location(curr_loc)
        # self.buy_tome(curr_loc)
        ...
        self.open_cube()
        tp_tome = self._template_finder.search_and_wait("TP_TOME", roi=self._config.ui_roi["right_inventory"], time_out=3, normalize_monitor=True) #we must search for the tome in our loot columns. I run with loot columns=4, so we take an ROI of 4x 40px = 160x160 from the left inventory.
