...
class Cows:
    ...
    def _stony_field(self)-> bool:      
        ...
        keepernumber = 0
        while not found:   
            found = self._template_finder.search_and_wait(["COWS_STONY_FIELD_0", "COWS_STONY_FIELD_1"], threshold=0.85, time_out=0.1, take_ss=False, use_grayscale=False).valid
        ...
        if found == True:
            ...
            roomfound = False
            badroom = False
            template_match = self._template_finder.search_and_wait(["COWS_STONY_FIELD_0", "COWS_STONY_FIELD_1"], best_match=True, threshold=0.8, time_out=0.1, use_grayscale=False)
            ...
            while not roomfound and not badroom:
                roomfound = self._template_finder.search_and_wait(["COWS_STONY_FIELD_PORTAL_0", "COWS_STONY_FIELD_PORTAL_1", "COWS_STONY_FIELD_PORTAL_2"], threshold=0.8, time_out=0.1, take_ss=False, use_grayscale=False).valid
                template_match = self._template_finder.search_and_wait(["COWS_STONY_FIELD_0", "COWS_STONY_FIELD_1"], best_match=True, threshold=0.8, time_out=0.1, use_grayscale=False)
                if template_match.valid:
                    pos_m = self._screen.convert_screen_to_monitor(template_match.position)
                t0 = self._screen.grab()
            ...
            if roomfound == True:
                ...
                if not self._char.select_by_template(["COWS_STONY_FIELD_PORTAL_1"], found_loading_screen_func, threshold=0.5, time_out=4):
                    ...
                    if not self._char.select_by_template(["COWS_STONY_FIELD_PORTAL_1"], found_loading_screen_func, threshold=0.3, time_out=4):
                        return False
