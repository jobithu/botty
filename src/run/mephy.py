...
class Meph:
    ...
    def wpthreetravel(self):
        ...
        self._char.move(pos_m, force_tp=True)
        self._char.move(pos_m, force_tp=True)
        pos_m = self._screen.convert_abs_to_monitor((-500, -10))
        self._char.move(pos_m, force_tp=True)
        self._char.move(pos_m, force_tp=True)
        self._char.move(pos_m, force_tp=True)
        score = 1
        ...
        while not found:    
            ...
            Logger.debug("derpin1")
            pos_m = self._screen.convert_abs_to_monitor((random.uniform(-300, -500), random.uniform(-50, -350)))
            t0 = self._screen.grab()
            self._char.move(pos_m, force_tp=True, force_move=True)
            t1 = self._screen.grab()
            # check difference between the two frames to determine if tele was good or not
            ...
            if score < .10:
                stuck_count += 1
                if stuck_count >= 2 and super_stuck < 15:
                    pos_m = self._screen.convert_abs_to_monitor((350, -350))
                    self._char.move(pos_m, force_tp=True)
                    self._char.move(pos_m, force_tp=True)
                    pos_m = self._screen.convert_abs_to_monitor((350, -350))
                    ...
                    Logger.debug("STUCK")
                    super_stuck +=1
                elif stuck_count >= 2 and super_stuck >= 15:
                    Logger.debug("Super stuck this little manuvuer will cost us... umm i dunno")
                    pos_m = self._screen.convert_abs_to_monitor((600, 350))
                    self._char.move(pos_m, force_tp=True)
                    self._char.move(pos_m, force_tp=True)
    ...
    def battle(self, do_pre_buff: bool) -> Union[bool, tuple[Location, bool]]:
        ...
        if layout == 'MEPH_LVL2_WP1' or layout == "MEPH_LVL2_WP1_0" or layout == "MEPH_LVL2_WP1_1" or layout == "MEPH_LVL2_WP1_2" or layout == "MEPH_LVL2_WP1_3" or layout == "MEPH_LVL2_WP1_4" or layout == "MEPH_LVL2_WP1_5" or layout == "MEPH_LVL2_WP1_6" or layout == "MEPH_LVL2_WP1_7" or layout == "MEPH_LVL2_WP1_8" or layout == "MEPH_LVL2_WP1_9":
            ...
            if template_match.valid:
                ...
                purple_tele = pos_m
            while not roomfound:
                roomfound = self._template_finder.search_and_wait(["MEPH_LVL2_WP2E_1", "MEPH_LVL2_WP2E_12", "MEPH_LVL2_WP2E_5", "MEPH_LVL2_WP2E_6", "MEPH_LVL2_WP2E_7", "MEPH_LVL2_WP2E_8", "MEPH_LVL2_WP2E_9", "MEPH_LVL2_WP2E_11"], threshold=0.8, time_out=0.1, take_ss=False, use_grayscale=False).valid
                t0 = self._screen.grab()
                pos_m = self._screen.convert_screen_to_monitor(template_match.position)
                self._char.move(pos_m, force_tp=True, force_move=True)
                t1 = self._screen.grab()
            ...
            if template_match.valid:
                ...
                pos_m = self._screen.convert_screen_to_monitor(template_match.position)
            while not roomfound:
                roomfound = self._template_finder.search_and_wait(["MEPH_EXIT2", "MEPH_LVL2_WP3E_1", "MEPH_LVL2_WP3E_0", "MEPH_LVL2_WP3E_2"], threshold=0.8, time_out=0.1, take_ss=False, use_grayscale=False).valid
                pos_m = self._screen.convert_screen_to_monitor(template_match.position)
                t0 = self._screen.grab()
                self._char.move(pos_m, force_tp=True, force_move=True)
                t1 = self._screen.grab()
